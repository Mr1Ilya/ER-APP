...
async fn ensure_authlib_injector(libraries_dir: &std::path::Path) -> crate::Result<std::path::PathBuf> {
    let injector_path = libraries_dir.join("authlib-injector.jar");
    if !injector_path.exists() {
        tracing::info!("Downloading authlib-injector.jar...");
        
        let client = reqwest::Client::builder()
            .timeout(std::time::Duration::from_secs(10))
            .build()
            .map_err(|e| crate::ErrorKind::LauncherError(format!("Failed to build HTTP client: {e}")))?;
        ...
        for url in urls.iter() {
            tracing::info!("Attempting download of authlib-injector from: {url}");
            match client.get(*url).send().await {
                Ok(resp) => {
                    if resp.status().is_success() {
                        match resp.bytes().await {
                            Ok(bytes) => {
                                if bytes.len() > 10000 {
                                    ...
                                    tracing::warn!("Downloaded file from {url} is too small: {} bytes", bytes.len());
                                }
                            }
                            Err(e) => {
                        ...
                        tracing::warn!("Request to {url} returned non-success status: {}", resp.status());
                    }
                }
                Err(e) => {
        ...
        let bytes = downloaded_bytes.ok_or_else(|| {
            crate::ErrorKind::LauncherError("Failed to download authlib-injector.jar from all available mirrors. Please check your internet connection.".to_string())
        })?;

        std::fs::write(&injector_path, bytes)
            .map_err(|e| crate::ErrorKind::LauncherError(format!("Failed to write authlib-injector: {e}")))?;
        tracing::info!("Successfully downloaded authlib-injector.jar!");
    }
    Ok(injector_path)
}
