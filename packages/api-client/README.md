...
```bash
pnpm add @erteam/api-client
```

Tauri apps also need the optional peer dependency:

```bash
pnpm add @erteam/api-client @tauri-apps/plugin-http
```
...
```ts
import { AuthFeature, GenericModrinthClient, type Labrinth } from '@erteam/api-client'
...
```ts
import { AuthFeature, CircuitBreakerFeature, NuxtCircuitBreakerStorage, NuxtModrinthClient } from '@erteam/api-client'
...
import { AuthFeature, TauriModrinthClient } from '@erteam/api-client'

const client = new TauriModrinthClient({
	userAgent: async () => `modrinth/theseus/${await getVersion()} (support@erteam.com)`,
	features: [new AuthFeature({ token: process.env.MODRINTH_TOKEN })],
})
...
```ts
import type { Labrinth } from '@erteam/api-client'
...
```ts
import { AuthFeature, CircuitBreakerFeature, RetryFeature } from '@erteam/api-client'
...
```bash
pnpm --filter @erteam/api-client build
pnpm --filter @erteam/api-client lint
