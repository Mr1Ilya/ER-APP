...
import { ButtonStyled, Collapsible, injectNotificationManager } from '@erteam/ui'
...
const supportLink = ref('https://end-rage.ru')
const metadata = ref({})
...
defineExpose({
	async show(errorVal, context, canClose = true, source = null) {
		...
		if (errorVal.message && errorVal.message.includes('Minecraft authentication error:')) {
			...
			supportLink.value = 'https://end-rage.ru'
			metadata.value.instanceId = context.instanceId
		} else if (source === 'state_init') {
			title.value = 'Error initializing EndRage Launcher'
			...
			<div class="markdown-body">
				<template v-if="errorType === 'minecraft_auth'">
					<template v-if="metadata.network">
						...
						<p>
							It looks like there were issues with the EndRage Launcher connecting to Microsoft's
							servers. This is often the result of a poor connection, so we recommend trying again
							to see if it works. If issues continue to persist, visit
							<a
								href="https://end-rage.ru"
							>
								our support page
							</a>
							to troubleshoot.
					...
					<template v-else-if="metadata.hostsFile">
						<h3>Network issues</h3>
						<p>
							The EndRage Launcher tried to connect to Microsoft / Xbox / Minecraft services, but the
				...
				<template v-if="errorType === 'directory_move'">
					<template v-if="metadata.readOnly">
						...
						<p>
							It looks like the EndRage Launcher is unable to write to the directory you selected.
					...
					<template v-else>
						<p>
							The EndRage Launcher is unable to migrate to the new directory you selected. Please
				...
				<template v-else-if="errorType === 'state_init'">
					<p>
						EndRage Launcher failed to load correctly. This may be because of a corrupted file, or
				...
				<template v-else-if="errorType === 'no_loader_version'">
					<p>The EndRage Launcher failed to find the loader version for this instance.</p>
