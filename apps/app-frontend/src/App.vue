...
import { renderString } from '@erteam/utils'
...
async function fetchNews() {
	try {
		news.value = await invoke('plugin:utils|get_telegram_news')
	} catch (e) {
		console.error('Failed to fetch Telegram news:', e)
	} finally {
...
async function setupApp() {
	...
	// Critical announcements are disabled for launcher independence
...
const updatePopupMessages = defineMessages({
	...
	meteredBody: {
		...
		defaultMessage: `EndRage Launcher v{version} is available now! Since you're on a metered network, we didn't automatically download it.`,
	},
	downloadedBody: {
		id: 'app.update-popup.body.download-complete',
		defaultMessage: `EndRage Launcher v{version} has finished downloading. Reload to update now, or automatically when you close EndRage Launcher.`,
	},
	linuxBody: {
		id: 'app.update-popup.body.linux',
		defaultMessage:
			'EndRage Launcher v{version} is available. Use your package manager to update for the latest features and fixes!',
	},
	reload: {
...
async function processPendingSurveys() {
	...
	let surveys = []
	// Surveys are disabled for launcher independence
				...
				<div class="sidebar-default-content" :class="{ 'sidebar-enabled': sidebarVisible }">
					...
					<div class="p-4 flex flex-col gap-3">
						<div class="flex items-center justify-between">
							<h3 class="text-base text-primary font-medium m-0 flex items-center gap-1.5">
								<svg class="size-4 text-[#24A1DE] shrink-0" viewBox="0 0 24 24" fill="currentColor">
									<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69.01-.03.01-.14-.07-.2-.08-.06-.19-.04-.27-.02-.12.02-1.96 1.24-5.54 3.65-.52.36-.99.53-1.41.52-.46-.01-1.35-.26-2.01-.48-.81-.27-1.46-.42-1.4-.88.03-.24.37-.49 1.03-.75 4.02-1.75 6.7-2.91 8.04-3.47 3.82-1.61 4.61-1.89 5.13-1.89.11 0 .37.03.54.17.14.12.18.28.2.45-.02.07-.02.16-.03.24z"/>
								</svg>
								Новости проекта
							</h3>
							<a href="https://t.me/ERTeamAPP" target="_blank" class="text-xs text-brand hover:underline font-semibold" style="color: var(--color-brand);">t.me/ERTeamAPP</a>
						</div>
						
						<div v-if="loadingNews" class="flex items-center justify-center py-6">
							<div class="animate-spin rounded-full h-6 w-6 border-t-2 border-brand"></div>
						</div>
						
						<div v-else-if="news.length === 0" class="text-secondary text-sm text-center py-4">
							Новостей пока нет.
						</div>
						...
						<div v-else class="flex flex-col gap-2.5 max-h-[50vh] overflow-y-auto pr-1 custom-scrollbar">
							<div v-for="item in news" :key="item.id" class="news-card">
								...
								<div class="news-text telegram-message" v-html="item.text"></div>
							</div>
						</div>
					</div>
				</div>
...
.telegram-message :deep(a) {
	color: var(--color-brand);
	font-weight: 600;
	text-decoration: none;
}
.telegram-message :deep(a:hover) {
	text-decoration: underline;
}
.telegram-message :deep(br) {
