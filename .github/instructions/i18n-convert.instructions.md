...
You are given a Nuxt/Vue single-file component (.vue). Your task is to convert every hard-coded natural-language string in the <template> into our localization system using vue-i18n with utilities from `@erteam/ui`.
...
- In the <script setup> block, import `defineMessage` or `defineMessages` from `@erteam/ui`.
...
- Render rich-text messages with `<IntlFormatted>` from `@erteam/ui` using named slots:
...
- For more complex child handling, use `normalizeChildren` from `@erteam/ui`:
...
- Import and use `useVIntl()` from `@erteam/ui`; prefer `formatMessage` for simple strings:
...
- Ensure imports from `@erteam/ui` are present: `defineMessage`/`defineMessages`, `useVIntl`, `IntlFormatted`, and optionally `normalizeChildren`.
